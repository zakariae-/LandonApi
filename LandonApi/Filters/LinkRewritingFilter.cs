...
{
    /*
     * To Generate an absolute URL for an instance of the Link class
     */
    public class LinkRewritingFilter : IAsyncResultFilter
    {
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            // Get all properties in the model with reflection
            var allProperties = model
                .GetType()
                // To access the type info for the model
                .GetTypeInfo()
                // Will get both the properties on the model and any properties that inherits from a base class
                .GetAllProperties()
                // We'll make sure to grab only the properties that we can read
                .Where(p => p.CanRead)
                .ToArray();
